...
namespace AC
{
	...
	public class KickStarter : MonoBehaviour
	{
		 ...
		 */
		public static void ResetPlayer (Player ref_player, int ID, bool resetReferences, Quaternion _rotation, bool keepInventory = false, bool deleteInstantly = false, bool replacesOld = false)
		{
			...
			if (ref_player)
			{
				...
				if (KickStarter.runtimeInventory)
				{
					...
					if (KickStarter.saveSystem != null && KickStarter.saveSystem.DoesPlayerDataExist (ID, false))
					{
						...
						saveSystem.AssignPlayerData (ID, loadNewInventory, loadNewCamera);
					}
